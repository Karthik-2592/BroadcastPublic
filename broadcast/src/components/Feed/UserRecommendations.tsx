...
export default function UserRecommendations({ users = [] }: { users?: UserSummary[] }) {
  ...
  return (
    <>
      {users.length !== 0 &&
        <Box sx={{ width: '100%', height: 280, p: 2.5, mb: 2, bgcolor: '#1a1a2e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 1, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <Typography variant="subtitle1" sx={{ fontWeight: 700, fontSize: '1.1rem', ml: 1 }}>People you might like</Typography>
          <Box sx={{ flex: 1, display: 'flex', gap: 4, minHeight: 0 }}>
            {users.map((user) => (
              <Card key={user.id} onClick={() => navigate(`/profile/${user.id}`)} sx={{ flex: 1, minWidth: 0, height: '100%', p: 1.5, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, bgcolor: '#1a1a2e', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', '&:hover': { borderColor: 'rgba(179,136,255,0.35)', bgcolor: 'rgba(179,136,255,0.08)' } }}>
                <Avatar src={user.profile_picture ?? undefined} sx={{ width: 96, height: 96, mb: 0.5, bgcolor: 'rgb(124, 77, 255)', color:'white' }}>{displayName(user).charAt(0).toUpperCase()}</Avatar>
